...
os.makedirs('ptbours', exist_ok=True)
...
#     f.create_dataset('tracings', data=tracings_buffer, chunks=(1, 4096, 12), dtype='i2')
#
...
with open('datasets/ptb/RECORDS', 'r') as f:
    for idx, p in enumerate(f):
        ...
        path = os.path.join("datasets", "ptb", p)
...
with h5py.File('ptbours/exams_part_0.hdf5', 'w') as f:
    f.create_dataset('exam_id', data=exam_ids_buffer, dtype='i4')
